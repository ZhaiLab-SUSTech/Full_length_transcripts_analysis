...
LastEditTime : 2020-10-31 16:22:39
@Description  : add tag to bam (PacBio version)
                tags:
...
def load_read_info(read_info):
    ...
    Return:
    {
        'm64030_191009_125259/7209164/ccs': 97,
        'm64030_191009_125259/14222186/ccs': 73,
        'm64030_191009_125259/16780074/ccs': 99,
    }

    '''
    logging.info('Load read info')

    df = pd.read_csv(read_info, sep='\t')
    df.drop_duplicates('read_id', False, inplace=True)
    df = df.loc[:, ['read_id', 'polya_length']]
    read_info_dict = {item[0]: item[1] for item in df.values}
...
def main(infile, outfile, read_info, bed_intersect):
    ...
    for read in inbam:
        if not read.is_unmapped \
            ...
            # 获取read的polyA长度
            read_tail_length = read_info_dict[read.query_name]
